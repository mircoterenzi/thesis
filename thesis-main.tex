...
\begin{abstract}
Nel campo informatico, la riusabilità del codice è un aspetto fondamentale, associato a numerosi vantaggi come la riduzione dei 
tempi di sviluppo e una maggiore affidabilità, poiché è sufficiente che il codice sia testato una sola volta. Neverlang è un 
framework progettato per lo sviluppo di linguaggi di programmazione che adotta questa filosofia come elemento chiave, favorendo una 
modellazione \textit{feature-oriented} volta a promuovere il riutilizzo del codice e, in particolare, dei componenti linguistici. 
Lo scopo di Neverlang è scomporre gli aspetti funzionali di un linguaggio in una serie di funzionalità, chiamate \textit{features}, 
per poi creare Domain-Specific Languages (DSL) che implementano solamente le caratteristiche necessarie.

L’obiettivo di questo progetto di tesi è ricreare le funzionalità di SQL in modo modulare e indipendente, rendendo possibile la 
costruzione di un DSL selezionando soltanto le caratteristiche richieste dal contesto. In questo documento viene illustrato l’intero 
processo di progettazione e sviluppo del progetto, spiegando e motivando le scelte progettuali adottate. Infine, vengono descritte 
le possibili estensioni future del progetto, evidenziando i potenziali scenari di utilizzo del linguaggio sviluppato.
\end{abstract}
...
%----------------------------------------------------------------------------------------
Il presente lavoro di tesi si concentra sull’utilizzo di Neverlang, un framework per la creazione di linguaggi di programmazione 
modulari, applicato alla modellazione di Domain-Specific Languages (DSL). I DSL sono linguaggi specializzati, progettati per 
rispondere a specifiche esigenze di un dominio applicativo, offrendo un approccio più naturale e ottimizzato rispetto ai linguaggi 
di programmazione \textit{general-purpose}. Attraverso Neverlang è possibile costruire DSL componendo moduli che rappresentano le 
varie caratteristiche del linguaggio, favorendo la riusabilità e l’estendibilità dei componenti.
...
Di seguito viene brevemente descritta la struttura dei capitoli che compongono la tesi. Nel \Cref{chap:contesto} è illustrato il 
...
necessarie per comprendere il lavoro svolto. Nel \Cref{chap:requisiti} sono elencati gli obiettivi del progetto insieme ad un’analisi 
...
come deve essere strutturato. Nel \Cref{chap:design} è descritta l’architettura del progetto, illustrando come è stato modellato il 
...
durante lo sviluppo. Infine, nel \Cref{chap:conclusioni} viene esaminata la valutazione del progetto, analizzando i risultati ottenuti 
...
\lstinputlisting[
    float,
    label={lst:neverlang-example},
    language=Neverlang,
    numbers=none,
    frame=none,
    caption={Esempio di implementazione introduttiva del costrutto \textit{if-else}.}
]{listings/neverlang.txt}

Al \Cref{lst:neverlang-example} è illustrato un esempio di implementazione del costrutto condizionale \textit{if-else} in Neverlang. Sono definiti 
...
\end{lstlisting}
Inizialmente, le condizioni definite tramite operatori relazionali vengono tradotte in predicati Java, poi aggregati utilizzando l’operatore logico \texttt{AND}. Di seguito viene riportata l’implementazione di questi due 
elementi\footnote{Per brevità, vengono mostrate solo le parti di codice rilevanti per l’esempio. In particolare, come operatore relazionale viene utilizzato \texttt{>}, mentre come operatore logico viene utilizzato \texttt{AND}.}:
...
\lstinputlisting[
    ...
    numbers=none,
    frame=none,
    caption={Implementazione dei ruoli di \textit{struct-checking} delle operazioni utilizzate all'interno dell'esempio.}
]{listings/check-struct.txt}
