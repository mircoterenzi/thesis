...
\begin{abstract}
    ...
    framework progettato per lo sviluppo di linguaggi di programmazione che adotta questa filosofia come elemento chiave, favorendo 
    una modellazione \textit{feature-oriented} volta a promuovere il riutilizzo del codice e, in particolare, dei componenti linguistici. 
    ...
    per poi creare Domain-Specific Languages (DSL) che implementano solamente le caratteristiche necessarie. Inoltre, l'utilizzo di 
    DSL permette di diminuire l'\textit{abstraction gap}, ossia la ``distanza'' tra lo spazio concettuale del problema e il linguaggio 
    utilizzato per codificarlo, ed il ``carico cognitivo'' necessario per imparare tale linguaggio.
...
%----------------------------------------------------------------------------------------
Il presente lavoro di tesi si concentra sull’utilizzo di Neverlang Language Workbench, un prototipo di ricerca sviluppato dal professor 
W. Cazzola e dai suoi collaboratori, utilizzato per studiare metodologie agili nello sviluppo di Domain-Specific Languages (DSL). 
Neverlang è un framework che facilita la definizione di linguaggi di programmazione in modo incrementale, consentendo la 
separazione tra sintassi e semantica. L’utilizzo del framework, unito a una corretta progettazione modulare, permette a ogni 
componente del linguaggio, chiamato \textit{slice}, di essere sviluppato, testato e sostituito in maniera indipendente 
\cite{Cazzola2023,Vacchi2015}.
